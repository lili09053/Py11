...
from ui17 import Ui_MainWindow

schools = []  # Список участвующих школ
classes = []  # Список участвующих классов
is_empty = True  # Указывает, пуст ли выбор класса
colors = [QColor(255, 215, 0), QColor(197, 201, 199), QColor(205, 157, 50), QColor(255, 255, 255)]  # Доступные цвета
max_score = 0
place = 1
...
class TableClass(QMainWindow, Ui_MainWindow):
    ...
    def setup_columns(self):  # Создаём столбцы для таблицы
        self.table.setColumnCount(3)  # Задаём кол-во столбцов в таблице
        ...
        widget_item = QTableWidgetItem()  # Столбец таблицы 3
        widget_item.setText("Логин")  # Столбец таблицы 3. Заносим название столбца
        self.table.setHorizontalHeaderItem(2, widget_item)  # Помещаем столбец в таблицу
        ...
        with open("rez.csv", encoding='utf-8') as r_file:
            ...
            for row in file_reader:  # Считывание данных из CSV файла
                if self.is_counted(row):
                    ...
                    color = self.select_color(int(row["Score"]))
                    self.set_elem(self.table.horizontalHeaderItem(0).text(), row, row_count, color)
                    self.set_elem(self.table.horizontalHeaderItem(1).text(), row, row_count, color)
                    self.set_elem(self.table.horizontalHeaderItem(2).text(), row, row_count, color)
                    row_count += 1  # Переход на следующую строку таблицы

    def set_elem(self, column, row, row_count, color):  # Заполнение элемента таблицы
        index = -1
        element = ""
        if column == self.table.horizontalHeaderItem(0).text():
            index = 0
            element = row["user_name"].split(" ")[3]
        if column == self.table.horizontalHeaderItem(1).text():
            index = 1
            element = row["Score"]
        if column == self.table.horizontalHeaderItem(2).text():
            index = 2
            element = row["login"]
        ...
        if self.school_number.currentText() != "Все":
            ...
            if self.class_number.currentText() != "Все":
                ...
                if school == self.school_number.currentText() and clss == self.class_number.currentText():
                    item.setText(element)  # Записываем в элемент общий балл
            else:
        ...
        self.table.setItem(row_count, index, item)  # Заносим элемент в таблицу

    def select_color(self, score):  # Заполняем цвет и заносим элемент в таблицу
        global max_score, place
        if score < max_score and place < 4:
            place += 1
            max_score = score
        return colors[place - 1]
